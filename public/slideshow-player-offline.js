function parseOfflineConfig() {
  const injected = window.OFFLINE_SLIDESHOW_DATA;
  if (injected && typeof injected === 'object' && Array.isArray(injected.slides)) {
    return injected;
  }
  try {
    const local = JSON.parse(localStorage.getItem('offline_slideshow_config') || 'null');
    if (local && Array.isArray(local.slides)) return local;
  } catch (_err) {
    // ignore
  }
  return null;
...
const config = parseOfflineConfig();
if (!config || !Array.isArray(config.slides) || config.slides.length === 0) {
  document.body.innerHTML = '<main class="app-shell"><section class="panel"><h2>No offline slideshow data found.</h2><p>Use Slideshow Builder and click Open Offline Player or Download Offline HTML.</p><a href="/slideshow.html">Back to Slideshow Builder</a></section></main>';
  throw new Error('Missing offline slideshow config');
}
...
const state = {
  ...
  musicTracks: Array.isArray(config.music) ? config.music.map((x) => x.url || x).filter(Boolean) : [],
  musicIndex: 0,
...
playerNameEl.textContent = String(config.name || 'Offline Slideshow');
...
function getSlidePath(slide) {
  return String(slide.path || slide.src || '');
}

function renderSingleSlide() {
  const slide = config.slides[state.index];
  if (!slide) return;
  slideImageEl.src = getSlidePath(slide);
